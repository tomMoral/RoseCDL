...
class MEGPopDataset(torch.utils.data.Dataset):
    ...
    def make_dataset(self):
        ...
        for root, _, files in os.walk(self.path):
            for file in files:
                all_subjects.append(str(Path(root) / file))
        ...
        for subject in all_subjects:
            ...
            shapes_time.append(np.load(subject).shape[1] - self.window)
    ...
    def __getitem__(self, idx):
        for i in range(self.shapes_time.shape[0]):
            if idx < self.shapes_time[i]:
                ...
                index = idx if i == 0 else idx - self.shapes_time[i - 1]
                return torch.tensor(
                    data_norm[
                        :,
        ...
        raise ValueError(f"No item found for idx {idx}")
