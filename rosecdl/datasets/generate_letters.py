...
def convert_str_to_png(text, margin=12):
    ...
    assert os.system(PDFCROP_CMD.format(name=filename, margin=margin)) == 0
...
def generate_text(n_atoms=5, text_length=3000, n_spaces=3, random_state=None):
    ...
    if random_state == "ICML":
        ...
        D_char = np.r_[rng.choice(chars, replace=False, size=n_atoms), [" "] * n_spaces]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ICML", action="store_true", help="Generate an data with ICML letters."
    )
    args = parser.parse_args()

    if args.ICML:
        print(generate_text_npy(n_atoms=4, text_length=5000, random_state="ICML"))
        raise SystemExit(0)
