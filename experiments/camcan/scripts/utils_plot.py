...
def unsplit_z(z):
    ...
    z = z.swapaxes(0, 1).reshape(1, n_channels, n_times_split * n_splits)
...
def plot_z_boxplot(
    ...
    df_z["id"] = df_z.index
    df_z = pd.wide_to_long(df_z, stubnames=["Values"], i="id", j="Atom").reset_index()[
        ["Atom", "Values"]
    ]
    ...
    if add_points:
        sns.stripplot(x="Atom", y="Values", data=df_z, size=2, color=".3", linewidth=0)
