...
def my_func(i, list_subjects_path):
    ...
    results = Parallel(n_jobs=n_jobs)(
        delayed(get_subject_z_and_cost)(subject_path, D_hat_sub, reg=LMBD)
...
def get_df_cost_cat(cat, list_subjects_path):
    ...
    results = Parallel(n_jobs=n_jobs)(
        delayed(get_subject_z_and_cost)(subject_path, D_cat, reg=LMBD)
        for subject_path in list_subjects_path
    )

    dic_res = []
    for res in results:
        dic_res.append(dict(subject_id=res[0], dict_fit=f"D_cat{cat}", cost=res[2]))

    df_cost_cat = pd.DataFrame(data=dic_res)
    df_cost_cat.to_csv("df_cost_cat1.csv")
    df_cost_cat["subject_id"] = df_cost_cat["subject_id"].apply(lambda x: x[4:])
...
results = Parallel(n_jobs=n_jobs)(
    delayed(get_subject_z_and_cost)(subject_path, D_all, reg=LMBD)
    for subject_path in list_subjects_path
)

dic_res = []
for res in results:
    dic_res.append(dict(subject_id=res[0], dict_fit="D_all", cost=res[2]))
