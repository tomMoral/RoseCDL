...
for subject in subjects_healthy[2:3]:
    ...
    cdl_params = dict(
        ...
        # Z-step parameters
        solver_z='dicodile',
        solver_z_kwargs={'max_iter': 10_000, 'eps': 1e-12},
    ...
    plt.ylabel('Lasso loss')
    plt.legend()
    plt.savefig(f'loss_{subject}.pdf')
    plt.show()
    plt.clf()
    ...
    plt.ylim(0, None)
    plt.legend()
    plt.savefig(f'acti_{subject}.pdf')
    plt.show()
    plt.clf()
