# %%
import numpy as np
from pathlib import Path
import torch

from utils_apnea import plot_temporal_atoms
from wincdl.datasets import PhysionetDataset, create_physionet_dataloader
from wincdl.wincdl import WinCDL

# %%

import argparse

parser = argparse.ArgumentParser()
parser.add_argument(
    "--group",
    type=str,
    choices=["a", "b", "c", "x"],
    help="group id to run the CDL on ('a': apnea, 'b': borderline apnea, 'c': control, 'x': test)",
    default="a",
)
args = parser.parse_args()
group_id = args.group
# group_id = "a"
print(f"Run population CDL on group {group_id}")
apnea_dataloader = create_physionet_dataloader(
    db_dir="./apnea-ecg", group_id=group_id, window=10_000, mini_batch_size=10
)

apnea_cdl = WinCDL(
    n_components=10,
    kernel_size=75,
    n_channels=1,
    lmbd=0.1,  # learned alphacsc with fixed reg at 0.1
    n_iterations=5_000,
    epochs=10,
    max_batch=20,  # 20 subjects for group a
    # mini_batch_window=10_000,
    # mini_batch_size=10,
    device="cuda:1",
    list_D=True,
)
# %%
losses, list_D, times = apnea_cdl.fit(apnea_dataloader, n_iter_eval=5_000)
np.save(f"d_hat_pop_{group_id}", apnea_cdl.D_hat_)
np.save(f"losses_pop_{group_id}", losses)
np.save(f"times_pop_{group_id}", times)
plot_temporal_atoms(apnea_cdl.D_hat_.squeeze(), save_fig=f"pop_{group_id}_atoms.pdf")
# %%
