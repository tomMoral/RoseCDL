...
if add_number:
    ...
    n_non_apnea = participants[participants["Record"].isin(subject_id_list)][
        "non-apn (minutes)"
    ].values
...
yy_self = [
    df_self[df_self["subject_id"] == xlabel]["cost"].values[0] for xlabel in xticklabels
]
plt.scatter(xticklabels, yy_self, marker="*", label="self")
# add chunck init
yy_init = [
    df_init[df_init["subject_id"] == xlabel]["cost"].values[0] for xlabel in xticklabels
]
...
if df_pop_cost is not None:
    yy_pop = [
        df_pop_cost[df_pop_cost["subject_id"] == xlabel]["cost"].values[0]
        for xlabel in xticklabels
    ]
...
if add_number:
    ...
    yy = n_non_apnea
    ax2.plot(xx, yy, color="black", alpha=0.6)
    ax2.set_ylim(0)
