...
def test_get_outlier_mask_1d():
    ...
    mask = get_outlier_mask(err, mehod="quantile", threshold=2 * len_outlier / len_sig)
    assert torch.all(mask == err)

    opening_window = 3
    mask = get_outlier_mask(err, mehod="quantile", threshold=0.8, opening_window=opening_window)
    expected_mask = torch.zeros(1, 1, len_sig)
    masked_times = slice(start_outlier-opening_window+1, start_outlier+len_outlier+1)
