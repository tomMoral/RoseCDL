...
class CSC1d(nn.Module):
    ...
    def rescale(self):
        ...
        with torch.no_grad():
            if self.rank == "uv_constraint":
                if self.window is not None:
                    norm_col_v = torch.norm(self.window * self.v,
                                            dim=2,
                                            keepdim=True)
                else:
                    norm_col_v = torch.norm(self.v, dim=2, keepdim=True)
                norm_col_v[torch.nonzero(
                    (norm_col_v == 0), as_tuple=False)] = 1
                self.v /= norm_col_v

                norm_col_u = torch.norm(self.u, dim=1, keepdim=True)
                norm_col_u[torch.nonzero(
                    (norm_col_u == 0), as_tuple=False)] = 1
                self.u /= norm_col_u
                return norm_col_v, norm_col_u

            elif self.rank == "full":
                if self.window is not None:
                    norm_atoms = torch.norm(self.window * self._D_hat,
                                            dim=(1, 2),
                                            keepdim=True)
                else:
                    norm_atoms = torch.norm(self._D_hat,
                                            dim=(1, 2),
                                            keepdim=True)
                norm_atoms[torch.nonzero(
                    (norm_atoms == 0), as_tuple=False)] = 1
