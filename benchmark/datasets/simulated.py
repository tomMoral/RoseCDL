...
class Dataset(BaseDataset):
    ...
    def get_data(self):
        ...
        X, D, _, outliers = simulate_1d(
            ...
            n_times=self.n_times, p_acti=0.7, p_contaminate=0.1,
        ...
        D_init = init_dictionary(
            X, n_atoms=self.n_atoms, n_times_atom=self.n_times_atom,
            rank1=False, window=True, D_init="random", random_state=rng.integers(0, 2**32)
        )

        return dict(
            X=X, D=D, D_init=D_init, outliers=outliers, window=True
        )
